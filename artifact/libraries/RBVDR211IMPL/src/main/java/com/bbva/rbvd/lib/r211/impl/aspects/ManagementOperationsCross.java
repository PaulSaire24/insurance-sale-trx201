...
public class ManagementOperationsCross implements ManagementOperation {
     ...
     *
     * @param contextEmission The ContextEmission object containing the details of the pre-policy process.
     */
    @Override
    public void afterProcessBusinessExecutionNotLifeCross(ContextEmission contextEmission){
        EmisionBO rimacResponse = contextEmission.getRimacResponse();
        PolicyDTO policy        = contextEmission.getPolicy();
        String policyNumber;
        if(Objects.nonNull(rimacResponse) && Objects.isNull(rimacResponse.getErrorRimacBO())) {
            ...
            Map<String, Object> argumentsRimacContractInformation = this.mapperHelper.getRimacContractInformation(rimacResponse, contextEmission.getAsoResponse().getData().getId());
            ...
            if (!Arrays.asList(productsCalculateValidityMonths.split(",")).contains(operacionGlossaryDesc)) {
                List<InsuranceCtrReceiptsDAO> otherReceipts = rimacResponse.getPayload().getCuotasFinanciamiento().stream().
                        filter(cuota -> cuota.getCuota().compareTo(1L) > 0).map(cuota -> InsuranceReceiptBean.generateNextReceipt(contextEmission.getAsoResponse(), cuota)).
                        collect(toList());
            ...
            String intAccountId = contextEmission.getAsoResponse().getData().getId().substring(10);
        ...
        this.mapperHelper.mappingOutputFields(policy, contextEmission.getAsoResponse(), rimacResponse, contextEmission.getRequiredFieldsEmission());
     ...
     */
    @Override
    public void afterProcessBusinessExecutionLifeCross(ContextEmission contextEmission){
        EmisionBO rimacResponse = contextEmission.getRimacResponse();
        PolicyDTO policy        = contextEmission.getPolicy();
        PolicyASO policyASO        = contextEmission.getAsoResponse();
